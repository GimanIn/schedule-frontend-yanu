...
import com.example.mycalendar.model.SignupRequest         // ✅ NEW: 모델 import
import com.example.mycalendar.model.SignupResponse        // ✅ NEW: 모델 import
import com.example.mycalendar.network.RetrofitClient      // ✅ NEW: Retrofit import
import com.example.mycalendar.network.ApiService          // ✅ NEW: Retrofit import
...
class SignupActivity2 : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val name = intent.getStringExtra("name") ?: ""     // ✅ NEW: name 받아오기
        val phone = intent.getStringExtra("phone") ?: ""   // ✅ NEW: phone 받아오기
        ...
        btnFinish.setOnClickListener {
            if (btnFinish.isEnabled) {
                ...
                val signupRequest = SignupRequest(userId, name, phone, passwordValue)  // ✅ NEW: 회원가입 요청 객체 생성
                val apiService = RetrofitClient.apiService                              // ✅ NEW: Retrofit 사용

                apiService.signup(signupRequest).enqueue(object : Callback<SignupResponse> {  // ✅ NEW
                    override fun onResponse(call: Call<SignupResponse>, response: Response<SignupResponse>) {
                        if (response.isSuccessful && response.body()?.success == true) {
                            val intent = Intent(this@SignupActivity2, SignupCompleteActivity::class.java)
